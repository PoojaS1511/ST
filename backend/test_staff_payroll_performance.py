#!/usr/bin/env python3
"""
Performance test for 2000+ staff payroll records.
...
def test_basic_performance():
    ...
    for page_size in page_sizes:
        ...
        def query_func():
            return supabase.table('finance_staffpayroll').select('*').limit(page_size).execute()
...
def test_pagination_performance():
    ...
    for page in range(1, total_pages + 1):
        ...
        def query_func():
            return supabase.table('finance_staffpayroll').select('*').range(
...
def test_filtering_performance():
    ...
    filters = [
        ("Department filter (Civil)", lambda: supabase.table('finance_staffpayroll').select('*').eq('department', 'Civil').limit(100).execute()),
        ("Department filter (Mechanical)", lambda: supabase.table('finance_staffpayroll').select('*').eq('department', 'Mechanical').limit(100).execute()),
        ("Role filter (Lab Assistant)", lambda: supabase.table('finance_staffpayroll').select('*').eq('role', 'Lab Assistant').limit(100).execute()),
        ("Role filter (Driver)", lambda: supabase.table('finance_staffpayroll').select('*').eq('role', 'Driver').limit(100).execute()),
        ("Salary filter (>= 50000)", lambda: supabase.table('finance_staffpayroll').select('*').gte('net_salary', 50000).limit(100).execute()),
        ("Salary filter (>= 70000)", lambda: supabase.table('finance_staffpayroll').select('*').gte('net_salary', 70000).limit(100).execute()),
        ("Search filter (name)", lambda: supabase.table('finance_staffpayroll').select('*').ilike('staff_name', '%a%').limit(100).execute()),
        ("Complex filter", lambda: supabase.table('finance_staffpayroll').select('*').eq('department', 'Civil').gte('net_salary', 40000).limit(100).execute()),
    ]
...
def test_summary_calculation_performance():
    ...
    summary_queries = [
        ("Staff ID only", lambda: supabase.table('finance_staffpayroll').select('staff_id').execute()),
        ("Net salary only", lambda: supabase.table('finance_staffpayroll').select('net_salary').execute()),
        ("All salary fields", lambda: supabase.table('finance_staffpayroll').select('base_salary, allowance, deduction, net_salary').execute()),
        ("All fields", lambda: supabase.table('finance_staffpayroll').select('*').execute()),
    ]
...
def test_concurrent_queries():
    ...
    queries = [
        ("Query 1 (100 records)", lambda: supabase.table('finance_staffpayroll').select('*').limit(100).execute()),
        ("Query 2 (200 records)", lambda: supabase.table('finance_staffpayroll').select('*').limit(200).execute()),
        ("Query 3 (500 records)", lambda: supabase.table('finance_staffpayroll').select('*').limit(500).execute()),
        ("Query 4 (Department filter)", lambda: supabase.table('finance_staffpayroll').select('*').eq('department', 'Civil').limit(100).execute()),
        ("Query 5 (Salary filter)", lambda: supabase.table('finance_staffpayroll').select('*').gte('net_salary', 50000).limit(100).execute()),
    ]
...
def test_memory_usage():
    ...
    try:
        ...
        for size in sizes:
            ...
            # Fetch data
            start_time = time.time()
            result = supabase.table('finance_staffpayroll').select('*').limit(size).execute()
            end_time = time.time()
...
def test_salary_calculation_performance():
    """Test salary calculation performance"""
    print("\n💰 SALARY CALCULATION PERFORMANCE TEST")
    print("=" * 60)
    
    supabase = get_supabase()
    
    # Test calculation performance with different record counts
    record_counts = [100, 500, 1000, 2000]
    
    for count in record_counts:
        print(f"\n💰 Testing salary calculations for {count} records:")
        
        start_time = time.time()
        result = supabase.table('finance_staffpayroll').select('base_salary, allowance, deduction, net_salary').limit(count).execute()
        ...
        if result and result.data:
            ...
            for record in result.data:
                ...
                if net_salary == calculated_net:
                    correct_calculations += 1
            
            calc_time = (time.time() - calc_start_time) * 1000
            total_time = fetch_time + calc_time
            
            print(f"   Fetch time: {fetch_time:.2f}ms")
            print(f"   Calculation time: {calc_time:.2f}ms")
            print(f"   Total time: {total_time:.2f}ms")
            print(f"   Records processed: {len(result.data)}")
            print(f"   Correct calculations: {correct_calculations}/{len(result.data)}")
            print(f"   Time per calculation: {calc_time / len(result.data):.4f}ms")

def main():
    """Main performance test function"""
    print("🚀 PERFORMANCE TEST FOR 2000+ STAFF PAYROLL RECORDS")
    print("=" * 60)
    ...
    tests = [
        ...
        ("Salary Calculation Performance", test_salary_calculation_performance),
    ...
    if passed == total:
        ...
        print("💡 The system can handle 2000+ staff payroll records efficiently.")
    else:
    ...
    print("🔍 Implement proper indexing on frequently filtered columns (department, role)")
    print("💰 Consider caching for salary calculations and summary data")
    print("💾 Monitor memory usage with large datasets")
    print("🔄 Use lazy loading for better user experience")
    print("⚡ Optimize salary calculations for bulk processing")
