...
const getAuthHeaders = () => {
  ...
  if (!token) {
    ...
    // Instead of redirecting, we'll let the ProtectedRoute handle the redirection
...
export const addStudent = async (studentData) => {
  try {
    ...
    if (!studentData.password) {
      // Generate a password using the first 4 characters of the name and the last 4 digits of the phone number
  ...
  try {
    ...
    if (updates.email || updates.phone) {
      validateStudentData({
        email: updates.email,
        phone: updates.phone,
        full_name: updates.full_name || 'dummy', // dummy value since it's required
...
export const deleteStudent = async (id) => {
  try {
    ...
    // Start a transaction for related data if needed
...
export const importStudentsFromCSV = async (csvText) => {
  try {
    ...
    // Check first student to understand data structure
    ...
    // Import in batches
    const BATCH_SIZE = 25; // Smaller batch size for better error handling
    const results = [];
