...
const Students = () => {
  ...
  const fetchStudents = async () => {
    if (!user?.id) {
      console.error('Cannot fetch students: No user ID available');
    ...
    try {
      setLoading(true);
      setError('');
      console.log('Fetching faculty assignments for user ID:', user.id);
      
      // Use the user's ID directly from the user object
      const facultyId = user.id;
      
      // First get the faculty's assignments to determine available filters
      ...
      // Use the first assignment as default
      const firstAssignment = assignmentsResponse.data[0];
      console.log('Using assignment:', firstAssignment);
      
      if (!firstAssignment) {
        throw new Error('Invalid assignment data received');
      }
      
      // Now fetch students for this assignment
  ...
  useEffect(() => {
    // Skip initial render if auth is still loading
    if (authLoading) {
      console.log('Auth still loading...');
      return;
    }

    // If we have a user with ID, fetch students
    if (user?.id) {
      console.log('User data available, fetching students...', { userId: user.id });
      fetchStudents();
    } 
            ...
            {students.length > 0 ? (
              students.map((student) => (
                <tr key={student.id} className="hover:bg-gray-50">
                    ...
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-10 w-10">
                        {student.profile_picture_url ? (
                          <img className="h-10 w-10 rounded-full" src={student.profile_picture_url} alt="" />
                        ) : (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${student.attendance_percentage >= 75 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
