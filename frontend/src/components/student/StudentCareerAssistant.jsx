...
const StudentCareerAssistant = () => {
  ...
  useEffect(() => {
    const fetchRoadmaps = async () => {
      try {
        const response = await fetch(`${API_URL}/api/roadmap/${user.id}`, {
          headers: {
        ...
        if (response.ok) {
          ...
          throw new Error(result.message || 'Failed to fetch roadmaps');
        }
      } catch (error) {
  ...
  const fetchRoadmapSteps = async (roadmapId) => {
    try {
      const response = await fetch(`${API_URL}/api/roadmap/steps/${roadmapId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      if (response.ok) {
        ...
        throw new Error(result.message || 'Failed to fetch roadmap steps');
      }
    } catch (error) {
  ...
  const handleUpdateStepStatus = async (stepId, newStatus) => {
    try {
      const response = await fetch(`${API_URL}/api/roadmap/steps/update-status`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        throw new Error(result.message || 'Failed to update step status');
      }
    } catch (error) {
  ...
  const handleGenerateRoadmap = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const roadmapsResponse = await fetch(`${API_URL}/api/roadmap/${user.id}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleSendMessage = async () => {
    ...
    setChatMessage('');
    setChatHistory(prev => [...prev, { message: userMessage }]);
    setChatLoading(true);
    
    try {
      const response = await fetch(`${API_URL}/api/roadmap/mentor/chat`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      if (response.ok) {
        setChatHistory(prev => [
          ...prev.slice(0, -1),
          { message: userMessage, reply: result.reply }
        ]);
      } else {
        throw new Error(result.message || 'Failed to get mentor response');
      }
    } catch (error) {
